    ...
    if form.is_valid():
        ...
        if len(users) != 0:
            ...
            if user['password'] == input_password:
                if user['enable']:
                    if user['role'] == 'user':
                        return redirect('/dashboard_app/user_dashboard?user=' + user['email'])
                    else:
...
def manage_questions(request):
    ...
    return render(request, 'manage_questions.html', {'display_questions': display_questions, 'options': options_list, 'user': user})
...
def services_and_security(request):
    ...
    app_name = request.GET.get('app_name')
    md5 = request.POST.get('md5')
    print(request.method,'method---md5',md5)
    if request.method == 'POST':
        answers = {k: v for k, v in request.POST.dict().items() if k != 'csrfmiddlewaretoken' and k != 'md5'}
...
def process_answer(app_name):
    ...
    data = {'final_count': final_count, 'risk_rating': risk_rating, 'category_wise_risk_rating': category_wise_risk_rating}
    return data
